...
### 3.1 工具模式（Unity 风格快捷键，v1.1）

工具栏按钮与快捷键双向同步；**在左侧层级树点选节点后直接按键即可切换工具操作**（输入框聚焦时除外）：

| 快捷键 | 工具 | 行为 |
|--------|------|------|
| Q | 平移 | 画布任意处按住拖拽平移视图，不选择节点 |
| W | 移动 | 拖动节点本体改 `position`；隐藏手柄；方向键 ±1px / Shift±10px 微调 |
| E | 旋转 | 拖动节点绕轴心旋转写回 `rotation`；吸附开启时按 5° 对齐，Shift 固定 15° |
| R | 缩放 | 拖角手柄等比、边手柄单轴写回 `scale`；拖本体等比（向右/上放大） |
| T | 矩形 | 默认工具：8 手柄 resize 写回 `size`，拖本体移动 |
| G | 网格开关 | 等价勾选「网格」 |
| S | 吸附开关 | 等价勾选「🧲吸附」 |
...
| 移动 | W 工具/T 工具下按住元素拖拽；方向键 ±1px；Shift+方向键 ±10px | `position`（逆向公式 §4.2） |
| 调整大小 | T 工具选中框 8 手柄拖拽（对边固定）；Shift+角手柄等比 | `size`（+必要时 `position` 联动） |
| 缩放 | R 工具手柄/本体拖拽；属性面板 sx/sy 数值输入（步进 0.1，范围 0.1~10） | `scale` |
| 旋转 | E 工具拖拽；属性面板 deg 输入 | `rotation` |
| 锚点 | 9 宫格预设（左上…右下+拉伸）点击；默认**补偿开启**（§4.3） | `anchors`（+补偿重算 `position/size`） |
| 轴心 | 9 点选择，同样保持视觉矩形补偿 | `pivot`（+补偿重算 `position`） |
| 吸附 | 开关 + 步长（默认 1px，可设 2/4/8px）；拖拽与手柄生效 | 舍入到步长 |
| 撤销/重做 | Ctrl+Z / Ctrl+Shift+Z；工具栏按钮 | JSON 快照栈（上限 50 步，结构级快照） |
| 保存 | Ctrl+S / 保存按钮 → POST /api/save | 成功后清除未保存标记 ● |
| 视图缩放 | 滚轮 25%~200%，空格+拖拽平移 / Q 工具常驻平移 | 仅视图，不动数据 |
...
| 反馈点 | 说明 |
|--------|------|
| 工具栏高亮 | 「🧲吸附 + 步长」合并为吸附组，开启时橙色描边高亮 |
| 画布徽标 | 画布视口右下角悬浮「🧲 吸附 Npx」徽标（画布外，不挡设计稿），关闭时隐藏 |
| 拖拽浮标 | 拖拽/手柄操作时贴节点显示实时数值（`x/y`、`w/h`、`sx/sy`、`∠deg`）；吸附发生修正的瞬间橙色高亮并带 🧲 前缀 |
| 网格联动 | 网格开启时间距自动跟随吸附步长（步长 ≥4px 时生效，过密回落 8px 基准） |
| 状态栏 | 常驻显示「工具: xx · 吸附 Npx」当前态 |
